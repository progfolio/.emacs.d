...
(setq straight-repository-branch "feat/el-get-recipe-support")
...
;;(require 'el-get)
;;(require 'edebug)
;;(edebug-instrument-function #'straight--convert-recipe)
...
If SEPARATE is non-nil, launch a separate instance of mpv."
    ...
    (make-process :name "*elfeed-mpv*"
                  :command `("nohup" ,(if separate "mpv" "umpv")
                             ,@(nreverse (mapcar (lambda (entry)
                                                   (elfeed-entry-link entry))
                                                 (elfeed-search-selected)))
                             ;; added to prevent nohup.out file
                             "&>/dev/null")
...
#+begin_src emacs-lisp
  (use-package miscellany
    :straight (miscellany :repo "git@192.168.1.180:emacs/miscellany.git"
                          :branch "master"
                          :build ("/usr/bin/emacs"
                                  "--batch"
                                  "--execute" "(progn (require 'ob-tangle) (org-babel-tangle-file \"./miscellany.org\"))"))
    ...
    (+general-global-window
      "f" '(my/toggle-maximize-buffer :which-key "toggle-maximize-buffer"))
    (+general-global-toggle
...
#+begin_src emacs-lisp
(use-package mu4e
  :straight (:source el-get :files (:defaults "mu4e/*.el"))
  :commands (mu4e mu4e-update-index)
  :custom
  (mu4e-mu-binary "~/.emacs.d/straight/repos/mu/mu/mu")
...
#+begin_src emacs-lisp
(use-package vterm
  :straight (:post-build ((setq vterm-always-compile-module t) (require 'vterm)))
