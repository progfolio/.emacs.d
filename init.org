...
(defvar elpaca-repos-directory (expand-file-name "sources/" elpaca-directory))
(defvar elpaca-order '(elpaca :repo "https://github.com/progfolio/elpaca.git"
                              :build
                              ((:after elpaca-git--clone
                                       (lambda (e)
                                         (elpaca--call-with-log
                                          e 0 "git" "remote" "set-url" "origin" "git@github.com:progfolio/elpaca.git")
                                         (elpaca--continue-build e)))
                               (:not elpaca-activate))
                              :ref "feat/refactor" :depth 1 :inherit ignore
...
(when (getenv "EXWM")
  ...
  (elpaca exwm
    ...
    (setopt exwm-input-global-keys
            `(([?\s-l] . windmove-right)
              ([?\s-L] . +exwm-move-window-right)
              ([?\s-h] . windmove-left)
              ([?\s-H] . +exwm-move-window-left)
              ([?\s-j] . windmove-down)
              ([?\s-J] . +exwm-move-window-down)
              ([?\s-k] . windmove-up)
              ([?\s-K] . +exwm-move-window-up)
              ([?\s-i] . +exwm-edit)
              ([?\s-r] . restart-emacs)
              ...
              ([?\s-\;] . (lambda () (interactive) (call-interactively #'consult-buffer)))
              (,(kbd "<XF86Launch2>")  .
    ...
    (defun +exwm-rename-buffer ()
      ...
      (exwm-workspace-rename-buffer
       (concat (when exwm-title
               ...
               (when exwm-title " | ") exwm-class-name " EXWM")))
    ...
    (setopt exwm-manage-configurations
            '(((string-match-p ".*Void.*Stranger.*" exwm-title) managed t floating nil tiling-mode-line nil fullscreen t)
  ...
  (elpaca desktop-environment
    ...
    (setq desktop-environment-volume-normal-increment "--allow-boost -i 2")
    (setq desktop-environment-volume-normal-decrement "--allow-boost -d 2")
...
(use-package gptel
  ...
  (defun +gptel-web-search (query)
    (let* ((server-url "http://192.168.1.180:8888/search")
           (encoded-query (url-hexify-string query))
           (full-url (format "%s?q=%s&format=json" server-url encoded-query))
           (url-request-method "GET")
           (result-string ""))
      (with-current-buffer (url-retrieve-synchronously full-url)
        (goto-char (point-min))
        ;; Skip HTTP headers to get to raw JSON body
        (delete-region (point-min) (1+ (goto-char url-http-end-of-headers)))
        (let* ((json-object-type 'hash-table)
               (json-array-type 'list)
               (json-key-type 'string)
               (parsed-data (json-read)))
          (let ((results (gethash "results" parsed-data)))
            ;; Map top 5 search entries into a clean context format for the LLM
            (setq result-string
                  (mapconcat
                   (lambda (item)
                     (format "Title: %s\nURL: %s\nContent: %s\n---"
  ...
  (gptel-make-tool
   :name "searxng_web_lookup"
   :description "Search the web using a local SearXNG metasearch instance. Use this when the user asks for current events, up-to-date data, or information you do not know."
   :function #'+gptel-web-search
   :args '((:name "query"
                  :type string
                  :description "The search engine query string"))
   :category "web-search")
  (setq gptel-backend
        (gptel-make-ollama "AMD"
          :host "192.168.1.160:11434"
          :stream t
          :models '(qwen3.8:27b mixtral:8x7b (llama3.3:70b :capabilities '((:tool-use)))))
        gptel-model 'qwen3.8:27b
        gptel-default-mode 'org-mode
        gptel-temperature 1.0
        gptel-include-reasoning nil
        gptel-extra-params '((options . ((think . :json-false)))))
...
(use-package consult
  ...
  (defvar +consult-source-tabs
    (list :name "Tabs"
          ...
          :items (lambda () (mapcar (lambda (tab) (alist-get 'name tab))
                                    (tab-bar--tabs-recent))))
    "EXWM tabs source.")
  (add-to-list 'consult-buffer-sources '+consult-source-tabs 'append)
  ...
  (defvar +consult-source-firefox
    (list :name "Firefox"
          ...
          :face 'consult-buffer
          :new (lambda (search)
                 (make-process :name (format "firefox: %s" search)
                               :buffer nil
                               :command (list "firefox" "--search" search)))
          :items (lambda ()
                   (let ((consult-buffer-filter nil))
                     (consult--buffer-query :include (list "firefox")
                                            :as #'buffer-name
                                            :mode 'exwm-mode)))))
  (add-to-list 'consult-buffer-sources '+consult-source-firefox 'append)
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(use-package doom-themes :ensure (:repo ("doomemacs/themes" . "doom-themes") :defer t))
#+end_src
** edebug
#+begin_quote
...
#+end_quote
I've put my elfeed database under version control.
        ...
        (with-current-buffer (find-file-noselect +elfeed-feed-file)
          (save-excursion
            (save-restriction
              ...
              (goto-char (point-min))
              (let ((found nil))
                (org-element-map (org-element-parse-buffer) t
                  (lambda (node) (when-let* ((props (car (cdr-safe node)))
                                             (standards (plist-get props :standard-properties))
                                             (tags (org-get-tags (aref standards 0)))
                                             ((member "elfeed" tags))
                                             ((not (member "ignore" tags)))
                                             (link (plist-get props :RAW-LINK)))
                                   (push (cons link (delq 'elfeed (mapcar #'intern tags)))
                                         found)))
                  nil nil t)
                (nreverse found))))))
...
(use-package miscellany
  :ensure `(miscellany
            ...
            :build
            (:after elpaca-source
                    ,(elpaca-defscript +miscellany-tangle (:type elisp)
                       (require 'ob-tangle)
...
#+begin_src emacs-lisp :lexical t
;;@TODO: fix bug in org-habit so we can use unibyte chars
...
(use-package pdf-tools
  :ensure `(pdf-tools :build (:before elpaca-build-link
                                     ,(elpaca-defscript +pdf-tools-build-server (:type system)
                                        ("pwd")
                                        (,(expand-file-name "./server/autobuild"))))
...
(use-package joymacs
  :ensure (joymacs :host github :repo "skeeto/joymacs"
                   :main "joydemo.el"
                   :build (:after elpaca--git-checkout-ref
                                  (lambda (e)
                                    (elpaca-with-dir e source
                                      (elpaca--call-with-log e 0 "make")
                                      (elpaca--continue-build e))))
...
(use-package helpa
  :defer t
  :ensure (:host github :main nil :build (:not elpaca-build-autoloads) :repo "progfolio/HELPA" :depth nil :protocol ssh))
#+end_src

** tunic
#+begin_src emacs-lisp :lexical t
(use-package tunic
  :ensure (:host github :repo "progfolio/tunic")
  :defer t
  :disabled t
  :config
